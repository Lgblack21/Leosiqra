...
export function WalletList({ accounts }: WalletListProps) {
  return (
    <div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1 custom-scrollbar">
        {accounts.map((acc) => (
          <div
            key={acc.id}
            className="shrink-0 w-40 rounded-2xl bg-white border border-slate-100 shadow-sm p-4"
          >
