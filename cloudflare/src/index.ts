...
// ─────────────────────────────────────────────────────────────────────────
// Pemasangan Shortcut iOS "1 tap": server yang merakit file .shortcut lengkap
// dengan Personal API Token pengguna sudah tertanam di dalamnya, sehingga di
// iPhone cukup buka Shortcuts → "Add Shortcut" tanpa merakit langkah apa pun.
// (Satu-satunya syarat sekali seumur hidup: Settings → Shortcuts → Allow
// Untrusted Shortcuts, karena shortcut yang dibuat di luar iPhone tidak
// bertanda tangan iCloud.)
// ─────────────────────────────────────────────────────────────────────────

const escapeXml = (value: string) =>
  value.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");

// Serialized text token biasa (string literal).
const wfText = (value: string) =>
  `<dict><key>WFSerializationType</key><string>WFTextTokenString</string><key>Value</key><dict><key>string</key><string>${escapeXml(
    value
  )}</string><key>attachmentsByRange</key><dict/></dict></dict>`;

// "Ask Each Time": iOS menanyakan nilai ini setiap kali shortcut dijalankan.
// U+FFFC (object replacement char) menandai posisi lampiran pada range {0, 1}.
const wfAsk = (prompt: string) =>
  `<dict><key>WFSerializationType</key><string>WFTextTokenString</string><key>Value</key><dict><key>string</key><string>￼</string><key>attachmentsByRange</key><dict><key>{0, 1}</key><dict><key>Type</key><string>Ask</string><key>Prompt</key><string>${escapeXml(
    prompt
  )}</string></dict></dict></dict></dict>`;

// Satu item dictionary (WFItemType 0 = teks) untuk header / body JSON.
const wfDictItem = (keyName: string, valueToken: string) =>
  `<dict><key>WFItemType</key><integer>0</integer><key>WFKey</key>${wfText(
    keyName
  )}<key>WFValue</key>${valueToken}</dict>`;

const wfDictField = (itemsXml: string) =>
  `<dict><key>WFSerializationType</key><string>WFDictionaryFieldValue</string><key>Value</key><dict><key>WFDictionaryFieldValueItems</key><array>${itemsXml}</array></dict></dict>`;

const buildShortcutPlist = (token: string, apiUrl: string) => {
  const headers = wfDictField(
    wfDictItem("Authorization", wfText(`Bearer ${token}`)) +
      wfDictItem("Content-Type", wfText("application/json"))
  );
  const body = wfDictField(
    wfDictItem("type", wfAsk("Jenis? Ketik: pengeluaran atau pemasukan")) +
      wfDictItem("amount", wfAsk("Nominal transaksi (angka saja)")) +
      wfDictItem("category", wfAsk("Kategori (mis. Makan, Transport)")) +
      wfDictItem("account", wfAsk("Nama akun/rekening (mis. BCA)")) +
      wfDictItem("note", wfAsk("Catatan (boleh dikosongkan)"))
  );

  const getUrlAction =
    `<dict><key>WFWorkflowActionIdentifier</key><string>is.workflow.actions.downloadurl</string>` +
    `<key>WFWorkflowActionParameters</key><dict>` +
    `<key>WFHTTPMethod</key><string>POST</string>` +
    `<key>WFURL</key>${wfText(apiUrl)}` +
    `<key>WFHTTPHeaders</key>${headers}` +
    `<key>WFHTTPBodyType</key><string>JSON</string>` +
    `<key>WFJSONValues</key>${body}` +
    `</dict></dict>`;

  const notifyAction =
    `<dict><key>WFWorkflowActionIdentifier</key><string>is.workflow.actions.notification</string>` +
    `<key>WFWorkflowActionParameters</key><dict>` +
    `<key>WFNotificationActionTitle</key>${wfText("Leosiqra")}` +
    `<key>WFNotificationActionBody</key>${wfText("Transaksi berhasil dicatat ✓")}` +
    `</dict></dict>`;
  ...
  return (
    ...
    `<key>WFWorkflowInputContentItemClasses</key><array>${inputClasses}</array>` +
    `<key>WFWorkflowActions</key><array>${getUrlAction}${notifyAction}</array>` +
    `</dict></plist>`
  );
};

// Tabel efemeral untuk menautkan link install ke token mentah selama <15 menit.
// Dibuat lazy agar tidak bergantung pada migrasi manual dijalankan lebih dulu.
let shortcutInstallsReady = false;
const ensureShortcutInstallsTable = async (env: Env) => {
  if (shortcutInstallsReady) return;
  await env.DB.prepare(
  ...
  shortcutInstallsReady = true;
};

// Dipanggil dari web (login via cookie): buat token fresh + link install sekali
// pakai, kembalikan deep link shortcuts:// yang membuka Shortcuts app.
async function handleShortcutInstall(request: Request, env: Env) {
  const session = await readSession(env, request);
  if (!session) {
    return json({ error: "Unauthorized" }, { status: 401 });
  }
  ...
  await env.DB.prepare(
    ...
    .bind(generateId(), session.user.id, tokenHash, "iPhone Shortcut", nowIso())
    .run();

  await ensureShortcutInstallsTable(env);
  const key = toBase64Url(crypto.getRandomValues(new Uint8Array(24)).buffer);
  const expiresAt = new Date(Date.now() + 15 * 60 * 1000).toISOString();
  await env.DB.prepare(
    "INSERT INTO shortcut_installs (key, user_id, raw_token, expires_at, created_at) VALUES (?, ?, ?, ?, ?)"
  )
    .bind(key, session.user.id, rawToken, expiresAt, nowIso())
    .run();

  const origin = new URL(request.url).origin;
  const fileUrl = `${origin}/api/member/leosiqra.shortcut?k=${key}`;
  const importUrl = `shortcuts://import-shortcut?url=${encodeURIComponent(fileUrl)}&name=${encodeURIComponent(
    "Input Transaksi Leosiqra"
  )}`;
  return json({ ok: true, fileUrl, importUrl });
}

// Diambil langsung oleh Shortcuts app iOS (tanpa cookie) memakai key di query.
async function handleShortcutPlist(request: Request, env: Env) {
  const url = new URL(request.url);
  ...
  const row = await env.DB.prepare(
    "SELECT raw_token, expires_at FROM shortcut_installs WHERE key = ?"
  )
    .bind(key)
    .first<{ raw_token: string; expires_at: string }>();

  if (!row || new Date(row.expires_at).getTime() < Date.now()) {
    return new Response(
      "Link install kadaluarsa. Buka lagi halaman Profile lalu tap Pasang Shortcut.",
      { status: 410 }
    );
  }

  const apiUrl = `${url.origin}/api/member/quick-transaction`;
  const plist = buildShortcutPlist(row.raw_token, apiUrl);
  return new Response(plist, {
    status: 200,
    headers: {
      "Content-Type": "application/octet-stream",
      "Content-Disposition": 'attachment; filename="Input Transaksi Leosiqra.shortcut"',
...
const worker = {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      // Pemasangan Shortcut iOS 1-tap (lihat handler untuk detail).
      if (url.pathname === "/api/member/shortcut-install" && request.method === "POST") {
        return await handleShortcutInstall(request, env);
      }

      // File .shortcut diambil oleh Shortcuts app iOS; publik (auth via key query).
      if (url.pathname === "/api/member/leosiqra.shortcut" && request.method === "GET") {
        return await handleShortcutPlist(request, env);
      }
